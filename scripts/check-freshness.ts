...
function main(): void {
  ...
  const metaRows = db.prepare('SELECT key, value FROM db_metadata').all() as {
    key: string;
    value: string;
  }[];
  ...
  const status = stale
    ? 'OVERDUE'
    : daysSinceBuilt > FRESHNESS_THRESHOLD_DAYS * 0.8
      ? 'Due soon'
      : 'Current';
  ...
  console.log(
    `Freshness: ${status} (${daysSinceBuilt} days since build, threshold ${FRESHNESS_THRESHOLD_DAYS})`
  );
}
