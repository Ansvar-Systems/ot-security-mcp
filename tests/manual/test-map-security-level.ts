...
if (sl2.length > 0) {
  ...
  console.log('Security levels:', sl2[0].security_levels.map(l => `SL-${l.security_level}`).join(', '));
}
console.log();
...
if (sl2.length > 0) {
  const req = sl2[0];
  console.log(`Requirement ${req.requirement_id} has ${req.security_levels.length} security levels:`);
